...
**Approach 2: Type-Specific Solution**
```ts
...
This is exactly where generics come in - they allow us to write type-safe, reusable functions that work with any type while maintaining full type safety

**Approach 3: Generic Solution**
```ts
type User = {
  id: number;
  name: string;
...
- The function fetchData uses two generic parameters `T` and `E with default values `unknown` and `Error`, respectively.
- `T` represents the type of the data expected from the API response.
- `E` represents the type of the error that might occur.
- The function returns a `FetchResult` object with a `data` property of type `T` and an `error` property of type `E`.

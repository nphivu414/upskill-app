...
2. New dependencies:
   ...
   - **Important notes:** 
      ...
      - Some of the key concepts used in the flight filter form component include:
      ...
      - Some of the key concepts of Zod validation schema include:
...
So far, we've covered the base slider field component. Let's move on to the `PriceRangeSlider` and `TimeRangeSlider` components.

5. Deep dive into <CodeTabTrigger section="flight-filter-form" tab="price-range-slider.tsx"/> file: This component is specifically for the price range slider in the flight filter form. It utilizes the `SliderField` component and formats price values for display.
   - **Props:** This component accepts the similar props as the `SliderField` component.
   - **Rendering:** 
      - As this component was wrappeed inside `FormProvider`, we can access the form state and control object directly using `useFormContext` hook and pass them to the `SliderField` component.
      - The `name` prop is used to connect the slider value to the form state. So we need to make sure the `name` prop matches the field name in the form data. Since we've built the form field with Typescript, you should always see the suggestions for the field name.
      ```tsx
         // !border[/priceRange/gm]
         <SliderField
            name="priceRange"
            ...
         />
      ...
      ```
      - The `maxPrice` and `minPrice` were get from the initial from values and passed to the `SliderField` component to set the slider range.
      - We also add the `startContent` and `endContent` to display the currency symbol and the price value on the slider.

   6. Deep dive into <CodeTabTrigger section="flight-filter-form" tab="time-range-slider.tsx"/> file: This component is specifically for the time range slider in the flight filter form. It utilizes the `SliderField` component to display and format time ranges.
   - **Props:** This component accepts the similar props as the `SliderField` component.
   - **Rendering:** This component is similar to the `PriceRangeSlider` component, but we need to use `renderCustomValue` props to format the time value before displaying it.
      - `DateFormatter` and `Time` class from `@internationalized/date` were used to format the time value, make it show the time in 24-hour format.
      - `renderCustomValue` function is used to make the slider display the time value as we formatted.

   7. Deep dive into <CodeTabTrigger section="flight-filter-form" tab="checkbox-group-field.tsx"/> file: This component is a reusable checkbox group component using React Hook Form's `Controller` to manage form state. It allows selection of multiple checkbox options with support for "Select All" functionality.

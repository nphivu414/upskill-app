...
1. Build Posts components:
   ...
   - **Deep dive into `posts.tsx` file:** <br/> <br/>
      ...
      - **Action event handlers:**
         - `getPostActionsConfig(stats)`: This function takes in a `stats` object that contains information about a post's statistics (like the number of comments, reposts, and likes). It returns a configuration object for different actions you can perform on a post. Each action has a handler function that you can add your own logic to, and some actions also display the count of how many times that action has been performed.
         ...
         - We decided to create action handlers at the `Posts` component level instead of the `PostCard` component level. Here are some reasons why:
      ...
      Here's an example of how you can create custom hooks for post actions:
         ```tsx
         // usePostActions.ts
